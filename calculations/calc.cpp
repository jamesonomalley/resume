...
#include "stackArray.h" // user defined version of stack
using namespace std;

// PROTOTYPES for functions to read and evaluate fully balanced expression.
...
void evaluate_stack_tops(stackArray<double>& numbers, stackArray<char>& operations);
...
// Library facilities used: cstring, iostream, stack
{
    ...
    stackArray<double> numbers;
    stackArray<char> operations;
    double number;
    char symbol;
...
(2+(3*5))
That evaluates to 17

Type a fully parenthesized arithmetic expression:
((2+3)*5)
That evaluates to 25

Type a fully parenthesized arithmetic expression:
(2+((3*5)/2))
That evaluates to 9.5

Type a fully parenthesized arithmetic expression:
(((2+3)*5)/2)
That evaluates to 12.5
*/